...
export default function Inventory({ data }: { data: ReturnType<typeof useERPData> }) {
  ...
  const handleSaveMovement = async () => {
    ...
    // Se o usuário estiver adicionando um novo motivo, salva ele primeiro
    if (isAddingNewReason && newReasonName.trim()) {
      try {
        await data.addInventoryReason(newReasonName.trim(), movementType);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Inventory Table */}
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden text-slate-900 border-b border-slate-100">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 space-y-4 text-slate-800">
              <div className="space-y-1">
                ...
                <select 
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:border-blue-400 text-slate-800"
              ...
              <div className="space-y-1">
                ...
                <input 
                  type="number"
                  placeholder="0.00"
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:border-blue-400 font-bold text-slate-800"
                  value={quantity}
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Motivo</label>
                <div className="flex gap-2">
                  <select 
                    className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:border-blue-400 text-slate-800 text-slate-800"
              ...
              {isAddingNewReason && (
                <div className="space-y-1 animate-in slide-in-from-top-2 duration-200">
                  ...
                  <input 
                    ...
                    className="w-full px-4 py-3 bg-blue-50 border border-blue-200 rounded-xl text-sm outline-none focus:border-blue-400 font-medium text-slate-800 text-slate-800"
                    value={newReasonName}
            ...
            <div className="p-6 bg-slate-50/50 border-t border-slate-100 flex justify-end gap-3 text-slate-800">
              <button 
                onClick={() => { setIsModalOpen(false); resetForm(); }}
